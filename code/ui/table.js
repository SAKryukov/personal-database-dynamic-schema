class Table {
    ...
    this.#parent = parent;
        ...
        if (contextMenu != null) {
            let lastPointerX = 0;
            let lastPointerY = 0;
            this.#table.onpointermove = event => {
                lastPointerX = event.clientX;
                lastPointerY = event.clientY;
            }; //this.#table.onpointermove
            this.#table.oncontextmenu = event => {
                const isPointer = event.button >= 0;
                if (isPointer)
                    contextMenu.show(event.clientX, event.clientY);
                else
                    contextMenu.show(lastPointerX, lastPointerY);
            }; //this.#table.oncontextmenu
            window.oncontextmenu = event => {
                return !this.#table.contains(event.target);
    ...
    #selectDown(page) {
        ...
        this.#select(this.#body.rows[rowIndex + 1].cells[newX], true); //SA???
    } //#selectDown
    #selectTop() {
